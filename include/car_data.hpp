...
#define URG_ANGLE 45.0		//道路に対してのURGの取付角度(deg)


//#define URG_ANGLE 48.5		//道路に対してのURGの取付角度(deg)
//#define URG_ANGLE 24.0		//道路に対してのURGの取付角度(deg)
//#define URG_ANGLE2 3.0		//水平のURGの取付角度(deg) 0が望ましい
#define URG_ANGLE2 0		//水平のURGの取付角度(deg) 0が望ましい

#define URG_HEIGHT 1.85	//URGの取付高さ（垂直）
//#define URG_HEIGHT 1.792	//URGの取付高さ（垂直）
#define URG_WIDTH 1.15		//URGの原点からの横方向へのずれ(正面から車両を見て右側が正)
#define URG_DEPTH (-0.260 - FRONTBAMPER_REARWHEEL)	//URGの原点からの奥行き方向へのずれ(前方から後方へが正）

#define IBEO_ANGLE 0.0		//道路に対してのIBEOの取付角度(deg)
#define IBEO_HEIGHT 0.630	//IBEOの取付高さ（垂直）
#define IBEO_WIDTH 0.0		//IEBOの原点からの横方向へのずれ(正面から車両を見て右側が正)
#define IBEO_DEPTH (0.04 - FRONTBAMPER_REARWHEEL)		//IBEOの原点からの奥行き方向へのずれ(前方から後方へが正）
...
#define CAR_HEIGHT 3.34		//車高
#define CAR_WIDTH 2.49		//全幅
#define CAR_DEPTH 11.99		//全長

#define WHEELBASE 5.905		//ホイールベース
#define FRONT_TREAD 2.055	//フロントトレッド
#define REAR_TREAD 1.855	//リアトレッド
#define FRONTBAMPER_REARWHEEL 8.2	//センサの原点（フロントバンパー）から後輪（後4輪の中心・オドメトリの中心）までの距離

#define STATBILITY_FACTOR 0.001
